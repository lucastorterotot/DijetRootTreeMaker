...
double getCorrectedPFIsolation(double isolation, double rho, float eta, IsolationType type) 
{
  float effectiveArea = getEffectiveArea(eta, type); 
    return max(isolation - rho*effectiveArea, 0.);
...
//   //------------------ Genphoton ----------------------------------- 
  ...
  nVtx_   = recVtxs->size(); // Nb of good vertices has to be redefined (for now this is just the bumber of vertices)
  run_    = iEvent.id().run();
  ...
  //------- store the object that launch the last filter in the HLT photon path
  ...
  for(; ielectron != electron->end() ;++ielectron,++ielectronsmeared, ie++){
   ...
   bool elecID = fabs(primaryVertex.z() - ielectron->vertex().z()) < 1.;
    ...
    elecID     &= ((int) ielectron->electronID("cutBasedElectronID-Fall17-94X-V1-loose") & 0x1);
...
// Compute chsMET only for event with 1 photon      
      if(nPhotonsTight_ == 1){
      double deltar = 0 ;
      int nlim = 0 ;
      
      //find the uncorrected photon for MEt footprint correction
  ...
  //find all the photon corresponding candidate  in the Pf candidates
...
// PUPPI
  ...
  if(redoJECs_)
	...
	{
	  double correction = 1.;
          if(!ijet->hasPFSpecific())continue;
...
//official 2017 from https://twiki.cern.ch/twiki/bin/viewauth/CMS/CutBasedPhotonIdentificationRun2
bool DijetTreeProducer::isValidPhotonLoose(const pat::PhotonRef& photonRef, const edm::Event& event, double generatorWeight)
{
   ...
   // return false;
   // #1: H/E
    isValid &= photonRef->hadronicOverEm() < 0.105;
    if (! isValid)
    return false;
    //#2: sigma ietaieta
    isValid &= (*full5x5SigmaIEtaIEtaMap)[photonRef] < 0.0103; //Official    
    if (! isValid)
    return false;
    ...
    isValid &= getCorrectedPFIsolation((*phoChargedIsolationMap)[photonRef], rho, photonRef->eta(), IsolationType::CHARGED_HADRONS)< 2.839;
    isValid &= getCorrectedPFIsolation((*phoNeutralHadronIsolationMap)[photonRef], rho, photonRef->eta(), IsolationType::NEUTRAL_HADRONS) < (9.188 + 0.0126*photonRef->pt()+0.000026*(photonRef->pt()*photonRef->pt() ) );
    isValid &= getCorrectedPFIsolation((*phoPhotonIsolationMap)[photonRef], rho, photonRef->eta(), IsolationType::PHOTONS) < (2.956 + 0.0035*photonRef->pt());
...
bool DijetTreeProducer::isValidPhotonMedium(const pat::PhotonRef& photonRef, const edm::Event& event, double generatorWeight)
{
   ...
   // return false;
   // #1: H/E
    isValid &= photonRef->hadronicOverEm() < 0.035;
    if (! isValid)
    return false;
    //#2: sigma ietaieta
    isValid &= (*full5x5SigmaIEtaIEtaMap)[photonRef] < 0.0103; //Official    
    ...
    isValid &= getCorrectedPFIsolation((*phoChargedIsolationMap)[photonRef], rho, photonRef->eta(), IsolationType::CHARGED_HADRONS) < 1.416;
    isValid &= getCorrectedPFIsolation((*phoNeutralHadronIsolationMap)[photonRef], rho, photonRef->eta(), IsolationType::NEUTRAL_HADRONS) < (2.491 + 0.0126*photonRef->pt()+0.000026*(photonRef->pt()*photonRef->pt() ) );
    isValid &= getCorrectedPFIsolation((*phoPhotonIsolationMap)[photonRef], rho, photonRef->eta(), IsolationType::PHOTONS) < (2.952 + 0.0035*photonRef->pt());
    isValid &= photonRef->passElectronVeto();
    if (! isValid)
    return false;
    isValid &= photonRef->r9() >0.90;
...
bool DijetTreeProducer::isValidPhotonTight(const pat::PhotonRef& photonRef, const edm::Event& event, double generatorWeight)
{
   ...
   // return false;
   // #1: H/E
    isValid &= photonRef->hadronicOverEm() < 0.020;
    if (! isValid)
    return false;
    //#2: sigma ietaieta
    isValid &= (*full5x5SigmaIEtaIEtaMap)[photonRef] < 0.0103; //Official    
    ...
    isValid &= getCorrectedPFIsolation((*phoChargedIsolationMap)[photonRef], rho, photonRef->eta(), IsolationType::CHARGED_HADRONS) < 1.158;
    isValid &= getCorrectedPFIsolation((*phoNeutralHadronIsolationMap)[photonRef], rho, photonRef->eta(), IsolationType::NEUTRAL_HADRONS) < (1.267 + 0.0126*photonRef->pt()+0.000026*(photonRef->pt()*photonRef->pt() ) );
    isValid &= getCorrectedPFIsolation((*phoPhotonIsolationMap)[photonRef], rho, photonRef->eta(), IsolationType::PHOTONS) < (2.065+0.0035*photonRef->pt());
    isValid &= photonRef->passElectronVeto();
    if (! isValid)
    return false;
    isValid &= photonRef->r9() >0.90;
...
bool DijetTreeProducer::isValidEndcapPhotonLoose(const pat::PhotonRef& photonRef, const edm::Event& event, double generatorWeight)
{
   ...
   // #1: H/E
    ...
    return false;
    //#2: sigma ietaieta
    isValid &= (*full5x5SigmaIEtaIEtaMap)[photonRef] < 0.03013; //Official    
    if (! isValid)
    return false;
    ...
    isValid &= getCorrectedPFIsolation((*phoChargedIsolationMap)[photonRef], rho, photonRef->eta(), IsolationType::CHARGED_HADRONS)< 1.011;
    isValid &= getCorrectedPFIsolation((*phoNeutralHadronIsolationMap)[photonRef], rho, photonRef->eta(), IsolationType::NEUTRAL_HADRONS) < (5.931 + 0.0163*photonRef->pt()+0.000014*(photonRef->pt()*photonRef->pt() ) );
    isValid &= getCorrectedPFIsolation((*phoPhotonIsolationMap)[photonRef], rho, photonRef->eta(), IsolationType::PHOTONS) < (6.641 + 0.0034*photonRef->pt());
    isValid &= photonRef->passElectronVeto();
    if (! isValid)
    return false;
    isValid &= photonRef->r9() >0.90;
...
bool DijetTreeProducer::isValidEndcapPhotonMedium(const pat::PhotonRef& photonRef, const edm::Event& event, double generatorWeight)
{
   ...
   // #1: H/E
    ...
    return false;
    //#2: sigma ietaieta
    isValid &= (*full5x5SigmaIEtaIEtaMap)[photonRef] < 0.03001; //Official    
    if (! isValid)
    return false;
    ...
    isValid &= getCorrectedPFIsolation((*phoChargedIsolationMap)[photonRef], rho, photonRef->eta(), IsolationType::CHARGED_HADRONS) < 0.442;
    isValid &= getCorrectedPFIsolation((*phoNeutralHadronIsolationMap)[photonRef], rho, photonRef->eta(), IsolationType::NEUTRAL_HADRONS) < (1.715 + 0.0163*photonRef->pt()+0.000014*(photonRef->pt()*photonRef->pt() ) );
    isValid &= getCorrectedPFIsolation((*phoPhotonIsolationMap)[photonRef], rho, photonRef->eta(), IsolationType::PHOTONS) < (3.863 + 0.0034*photonRef->pt());
    isValid &= photonRef->passElectronVeto();
    if (! isValid)
    return false;
    isValid &= photonRef->r9() >0.90;
...
bool DijetTreeProducer::isValidEndcapPhotonTight(const pat::PhotonRef& photonRef, const edm::Event& event, double generatorWeight)
{
   ...
   // #1: H/E
    ...
    return false;
    //#2: sigma ietaieta
    isValid &= (*full5x5SigmaIEtaIEtaMap)[photonRef] < 0.03000; //Official    
    if (! isValid)
    return false;
    ...
    isValid &= getCorrectedPFIsolation((*phoChargedIsolationMap)[photonRef], rho, photonRef->eta(), IsolationType::CHARGED_HADRONS) < 0.034;
    isValid &= getCorrectedPFIsolation((*phoNeutralHadronIsolationMap)[photonRef], rho, photonRef->eta(), IsolationType::NEUTRAL_HADRONS) < (0.585 + 0.0163*photonRef->pt()+0.000014*(photonRef->pt()*photonRef->pt() ) );
    isValid &= getCorrectedPFIsolation((*phoPhotonIsolationMap)[photonRef], rho, photonRef->eta(), IsolationType::PHOTONS) < (2.617+0.0034*photonRef->pt());
    isValid &= photonRef->passElectronVeto();
    if (! isValid)
    return false;
    isValid &= photonRef->r9() >0.90;
